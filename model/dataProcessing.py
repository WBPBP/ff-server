...
def balanceCheck(static_pressure_sum):
    ...
    comment=""
    if static_pressure_sum[0] < 0.45: # '왼-오'로 계산을 했기 때문에 압력 차의 평균 값이 양수인 경우 왼쪽으로 치우쳐져 있음
        comment = "몸의 무게중심이 왼쪽으로 치우쳐져 있는 경향이 보입니다."
    elif static_pressure_sum[0] < 0.55: # 압력 차의 평균이 음수인 경우 오른쪽으로 치우쳐져 있음
        comment = "몸의 무게중심이 오른쪽으로 치우쳐져 있는 경향이 보입니다."
    else: # 압력 차의 평균이=0인 경우 무게중심이 잘 잡혀있음
        comment = "몸무게가 양발에 고르게 분포해있어 몸의 균형은 안정적으로 보입니다."
    if static_pressure_sum[1]==1.0: #압력 차이가 기준보다 높은 것들의 수가 전체 개수의 반 이상이면 척추측만증 의심
...
def run(static_pressure_sum, footstep_pressure): #전체 수행 함수(main이 안될 경우를 대비해 함수로 만들어 놓았어요!)
    static_comment = balanceCheck(static_pressure_sum);
    per, gait_comment, diseaseNum = walkCheck(footstep_pressure)

    dict = {"staticPressureRes": static_comment, "percent": per, "gaitComment": gait_comment, "diseaseNum": diseaseNum}
    '''
...
static_pressure_sum.append(random.random())
static_pressure_sum.append(1.0)
run(static_pressure_sum, footstep_pressure)
